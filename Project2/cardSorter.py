...
class CardSorter():
            ...
            while flag < 5:
                ...
                if flag == 0:
                    ...
                    if len(self.__piles) <= 1:
                        ...
                        prev_card = self.__piles[len(self.__piles)-2].get_top_card()
                    self.add_card_to_subseq(card_from_deck, prev_card)
    ...
    def add_card_to_subseq(self, current_card, prev_card):
        ...
        if prev_card_subseq is None:
            ...
            first_card = self.__subseqs[prev_card_subseq].first_card()
            if first_card.data.compare(prev_card) == 0:
                self.__subseqs[prev_card_subseq].add_card(copied_card)

            else:
                subseq_copy = self.__subseqs[prev_card_subseq].clone_subseq()
                self.__subseqs.append(subseq_copy)

                for c in range(len(subseq_copy)):
                    card = self.__subseqs[len(self.__subseqs)-1].first_card()
                    if card.data.compare(prev_card) == 0:
                        break
                    else:
                        self.__subseqs[len(self.__subseqs)-1].remove_first()

                self.__subseqs[len(self.__subseqs)-1].add_card(copied_card)
    ...
    def display_longest_subseq(self):
        ...
        for i in range(len(self.__subseqs)):
            if len(self.__subseqs[i]) == len(self.__piles):
                cloned_subseq = self.__subseqs[i].clone_subseq()
                reversed_subseq = cloned_subseq.reverse_subseq()
                print(reversed_subseq)
                break
    ...
    def make_sorted_deck(self):
        ...
        for i in range(52):
            ...
            low_card = Card(3, 13)
            for t in range(len(self.__piles)):
                if (self.__piles[t].is_empty() == False):
                    first_card = self.__piles[t].get_top_card()
                    if first_card.compare(low_card) <= 0:
                        low_card = first_card
                        low_pile = t
            self.__deck.enqueue(self.__piles[low_pile].remove_card())